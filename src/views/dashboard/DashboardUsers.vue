<template>
  ...
  <div v-if="!route.params.id && route.name !== 'dashboard-users-create'">
    <AdminDataTable
      :title="t('dashboard.users.tableTitle')"
      :columns="columns"
      :items="userApi.users"
      :loading="userApi.loading.search"
      :total-count="userApi.totalCount"
      :current-page="pagination.page"
      :page-size="pagination.pageSize"
      @update:sort="updateSort"
      @pagination-change="handlePaginationChange"
      @apply-filters="loadUsers"
    ...
    >
      ...
      <template #filters>
        ...
        <DateRangeFilter
          :title="t('filters.user.createdAt')"
          v-model="createdAtRange"
          @date-for-server="updateCreatedAtDates"
          class="compact-filter"
        >
          <template #icon>
        ...
        <DateRangeFilter
          :title="t('filters.user.birthDate')"
          v-model="birthDateRange"
          @date-for-server="updateBirthDateDates"
          class="compact-filter"
        >
          <template #icon>
            <IconsSet name="calendar" class="w-4 h-4 text-primary-400" />
      ...
      <template #cell-role="{ item }">
        <span
          ...
          :class="
            Number(item.role) === UserRoles.Admin
    ...
    <ConfirmModal
      ...
      :title="t('dashboard.users.deleteConfirmTitle')"
      :message="t('dashboard.users.deleteConfirmMessage')"
...
const filter = reactive<UserFilterRequest>({
  ...
  email: null,
  phone: null,
  roles: [], // Changed role to roles as array of UserRoles
  birthDateFrom: null,
  birthDateTo: "",
  createdAtFrom: null,
  createdAtTo: "",
...
const resetFilters = () => {
  ...
  Object.assign(filter, {
    ...
    email: null,
    phone: null,
    roles: [], // Reset to empty array instead of null
    birthDateFrom: null,
    birthDateTo: "",
    createdAtFrom: null,
    createdAtTo: "",
