...
function getValidators(): Record<string, (v: string, p?: string) => string | boolean> {
  return {
    ...
    minLength: (v, p) =>
      ...
      t("validation.minLength", { length: p }),
    maxLength: (v, p) =>
      v.length <= +(p || 0) ||
      props.errorMessages.maxLength ||
      t("validation.maxLength", { length: p }),
    min: (v, p) =>
      +v >= +(p || 0) || props.errorMessages.min || t("validation.min", { min: p }),
    max: (v, p) =>
      +v <= +(p || 0) || props.errorMessages.max || t("validation.max", { max: p }),
    pattern: (v, p) =>
      new RegExp(p || "").test(v) ||
    ...
    match: (v) => {
      if (props.compareWith == null)
        return props.errorMessages.match || "No reference value for comparison";
      if (v !== props.compareWith)
        return props.errorMessages.match || t("validation.password.mismatch");
      return true;
    },
    name: (v) => {
      if (v.length < 3)
        return props.errorMessages.nameMinLength || t("validation.name.minLength");
      if (v.length > 50)
        return props.errorMessages.nameMaxLength || t("validation.name.maxLength");
      if (!/^[A-ZА-ЯЁ]/.test(v))
        return props.errorMessages.nameUppercase || t("validation.name.uppercase");
      if (!customPatterns.name.test(v))
        return props.errorMessages.namePattern || t("validation.name.pattern");
      return true;
    },
    lastName: (v) => {
      if (v.length < 3)
        return (
          props.errorMessages.lastNameMinLength ||
          t("validation.minLength", { length: 3 })
        );
      if (v.length > 50)
        return props.errorMessages.lastNameMaxLength || t("validation.name.maxLength");
      if (!/^[A-ZА-ЯЁ]/.test(v))
        return (
          props.errorMessages.lastNameUppercase || t("validation.lastName.uppercase")
        );
      if (!customPatterns.lastName.test(v))
        return props.errorMessages.lastNamePattern || t("validation.lastName.pattern");
      return true;
    },
    email: (v) =>
    ...
    phone: (v) =>
      ...
      t("validation.phone.pattern"),
    password: (v) => {
      if (v.length < 8)
        return (
          props.errorMessages.passwordMinLength ||
          t("validation.password.minLength", { length: 8 })
        );
      if (!/[A-ZА-ЯЁ]/.test(v))
        return (
          props.errorMessages.passwordUppercase || t("validation.password.uppercase")
        );
      if (!/[!@#$%^&*]/.test(v))
        return props.errorMessages.passwordSpecial || t("validation.password.special");
      if (!/\d/.test(v))
        return props.errorMessages.passwordDigit || t("validation.password.digit");
      if (!customPatterns.password.test(v))
        return props.errorMessages.passwordPattern || t("validation.password.pattern");
      return true;
    },
    ...
    passport: (v: string) => {
      if (!v)
        return props.errorMessages.passportRequired || t("validation.passport.required");
      if (!/^\d{4}\s?\d{6}$/.test(v))
        return props.errorMessages.passportPattern || t("validation.passport.pattern");
      return true;
    },
    ...
    birthDocument: (v: string) => {
      if (!v)
        return (
          props.errorMessages.birthDocumentRequired ||
          t("validation.birthDocument.required")
        );
      if (!/^[IVXLCDM]{1,5}-\d{6}$/.test(v)) {
        // Формат может быть вида "IV-123456" — серия римскими цифрами, затем номер
        return (
          props.errorMessages.birthDocumentPattern ||
          t("validation.birthDocument.pattern")
        );
      }
      return true;
    },
    ...
    driverLicense: (v: string) => {
      if (!v)
        return (
          props.errorMessages.driverLicenseRequired ||
          t("validation.driverLicense.required")
        );
      if (!/^\d{4}\s?\d{6}$/.test(v))
        return (
          props.errorMessages.driverLicensePattern ||
          t("validation.driverLicense.pattern")
        );
      return true;
    },
    ...
    studentCard: (v: string) => {
      if (!v)
        return (
          props.errorMessages.studentCardRequired || t("validation.studentCard.required")
        );
      if (!/^[А-Яа-яA-Za-z0-9\-]{5,20}$/.test(v)) {
        // Допускаются буквы, цифры, дефис — общая маска для студенческого
        return (
          props.errorMessages.studentCardPattern || t("validation.studentCard.pattern")
        );
      }
      return true;
    },
    fullName: (v) => {
      ...
      if (words.length < 2 || words.length > 3) {
        return (
          props.errorMessages.fullNameWordsCount || t("validation.fullName.wordsCount")
        );
      }
      ...
      for (const word of words) {
        ...
        if (word.length < 3) {
          return (
            props.errorMessages.fullNameWordLength || t("validation.fullName.wordLength")
          );
        }
        ...
        if (!/^[A-ZА-ЯЁ]/.test(word)) {
          return (
            props.errorMessages.fullNameUppercase || t("validation.fullName.uppercase")
          );
        }
        ...
        if (!/^[A-Za-zА-ЯЁа-яё]+$/.test(word)) {
          return (
            props.errorMessages.fullNameLettersOnly ||
            t("validation.fullName.lettersOnly")
          );
        }
      }
