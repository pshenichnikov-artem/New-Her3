<template>
  <div class="min-h-screen flex flex-col">
    ...
    <div class="flex-grow flex items-center justify-center bg-content py-16 px-8">
      <div
        class="w-full max-w-md p-8 bg-form border border-form-border rounded-lg shadow-lg"
      >
        <div class="flex justify-end mb-4">
          <LanguageSwitcher />
        </div>
        <h2 class="text-2xl font-bold text-text-accent mb-6">
          {{ t("pages.register") }}
        </h2>
        <form @submit.prevent="form.handleSubmit(onSubmit)">
          ...
          <ValidationInput
            ...
            @valid="form.updateValidationState('confirmPassword', $event)"
