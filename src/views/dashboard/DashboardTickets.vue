    ...
    <div v-if="!route.params.id && route.name !== 'dashboard-tickets-create'">
        <AdminDataTable :title="t('dashboard.tickets.tableTitle')" :columns="columns" :items="ticketApi.tickets"
            ...
            <template #filters>
                ...
                <TextFilter :title="t('filters.ticket.attendeeName')" v-model="filter.attendeeName"
                    class="compact-filter">
                    <template #icon>
                        <IconsSet name="user" class="w-4 h-4 text-primary-400" />
                    </template>
                </TextFilter>
                <SelectFilter :title="t('filters.ticket.status')" v-model="filter.status" :options="statusOptions"
                    :multiple-select="true" class="compact-filter">
                    <template #icon>
                        <IconsSet name="tag" class="w-4 h-4 text-primary-400" />
                    </template>
                </SelectFilter>
                <TextFilter :title="t('filters.ticket.eventId')" v-model="filter.eventIds" :multiple-select="true"
                    class="compact-filter">
                    <template #icon>
                        <IconsSet name="calendar" class="w-4 h-4 text-primary-400" />
                    </template>
                </TextFilter>
                <TextFilter :title="t('filters.ticket.buyerId')" v-model="filter.buyerIds" :multiple-select="true"
                    class="compact-filter">
                    <template #icon>
                        <IconsSet name="user" class="w-4 h-4 text-primary-400" />
                    </template>
                </TextFilter>
                <TextFilter :title="t('filters.ticket.attendeeId')" v-model="filter.attendeeId" :multiple-select="true"
                    class="compact-filter">
                    <template #icon>
                        <IconsSet name="user" class="w-4 h-4 text-primary-400" />
                    </template>
                </TextFilter>
                <TextFilter :title="t('filters.ticket.paymentId')" v-model="filter.paymentIds" :multiple-select="true"
                    class="compact-filter">
                    <template #icon>
            ...
            <template #cell-id="{ item }">
                <span class="text-white cursor-pointer select-none hover:underline transition duration-200"
                    title="Скопировать ticketId" @click="handleCopy(item.id)">
        ...
        <ConfirmModal v-if="isDeleteConfirmationVisible" :title="t('dashboard.tickets.deleteConfirmTitle')"
            :message="t('dashboard.tickets.deleteConfirmMessage')" :confirm-text="t('common.buttons.delete')"
