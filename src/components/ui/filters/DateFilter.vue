<template>
    <BaseFilterWrapper :title="title">
        ...
        <div v-if="multipleSelect && selectedValues.length > 0" class="flex flex-wrap mb-2">
            <FilterTag v-for="(value, index) in selectedValues" :key="index" :label="formatDate(value)"
        ...
        <div class="relative date-time-picker">
            <div @click="toggleCalendar"
                class="w-full border border-gray-300 rounded-lg px-3 py-2.5 bg-gray-50 flex items-center justify-between cursor-pointer hover:bg-white hover:border-indigo-300 transition-all">
                <span class="text-gray-500">{{ placeholder || t('datePicker.selectDate') }}</span>
...
const handleMultipleDateSelect = (date: Date) => {
    ...
    const existingDateIndex = selectedValues.value.findIndex(
        d => {
            const existingDate = new Date(d);
            return existingDate.getFullYear() === date.getFullYear() &&
