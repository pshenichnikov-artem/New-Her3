<template>
    <BaseEditor :title="isEditMode ? t('attendee.edit') : t('attendee.create')" :back-path="'/dashboard/attendees'"
        :is-loading="attendeeApi.isLoading" :has-changes="hasChanges" @back="goBack" @cancel="resetForm"
        ...
        <div v-else-if="attendeeForm">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <!-- Основная информация -->
                <div class="space-y-2">
                    <ValidationInput id="fullName" v-model="attendeeForm.fullName"
                        :label="t('attendee.fields.fullName')" validation-rules="required|fullName"
                        :readonly="isReadOnly" :error-messages="{
                    ...
                    <ValidationInput id="birthDate" v-model="attendeeForm.birthDate"
                        :label="t('attendee.fields.birthDate')" type="date" validation-rules="required|date"
                        :readonly="isReadOnly" :min="minBirthDate" :max="maxBirthDate" :error-messages="{
                            required: t('validation.required'),
                            date: t('validation.date'),
                            dateRange: t('validation.birthDate.range', { min: '-100', max: '-1' })
                        }" :trigger-validation="form.validationTrigger.birthDate" @valid="validateBirthDate" />
                </div>

                <!-- Документы и информация -->
                <div class="space-y-2">
                    <div class="form-group">
                        <label for="documentType" class="block mb-1 font-medium text-sm text-text-form">
                            {{ t('attendee.fields.documentType') }}
                        </label>
                        <select id="documentType" v-model="attendeeForm.documentType"
                    ...
                    <ValidationInput id="documentNumber" v-model="attendeeForm.documentNumber"
                        :label="t('attendee.fields.documentNumber')" :validation-rules="getDocumentValidationRules()"
                        :readonly="isReadOnly" :error-messages="getDocumentErrorMessages()"
...
const resetForm = () => {
    if (initialAttendeeData.value) {
        attendeeForm.value = { ...initialAttendeeData.value } as AttendeeUpdateRequest & { userId?: string | null };
    }
...
const saveAttendee = async () => {
    ...
    if (isEditMode.value) {
        ...
        const addData: AttendeeAddRequest = {
            ...
            documentNumber: attendeeForm.value.documentNumber,
            userId: null,
            ticketId: null // Передаем null для ticketId
        };
